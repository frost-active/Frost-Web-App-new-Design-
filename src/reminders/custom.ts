...
export function buildCustomReminder(config: DeviceConfig): EventReminder {
  ...
  const groups = source.events.map((event: any) => ({
    name: event.label,
    times: [event.h + event.m / 60],
    days: (event.days || []).map(dayIndex).filter((day: number) => day >= 0).sort((a: number, b: number) => a - b),
    enabled: event.enabled,
    dur: event.show_ms / 60000,
    type: event.type === 'absolute' ? 'absolute' : 'recurring',
